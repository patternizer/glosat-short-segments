...
#-----------------------------------------------------------------------
# PROGRAM: short-segment-analysis-arctic.py
#-----------------------------------------------------------------------
# Version 0.3
# 2 May, 2021
# Dr Michael Taylor
...
dt_continent_file = 'dt_arctic.csv'
da_continent_file = 'da_arctic.csv'
ds_continent_file = 'ds_arctic.csv'
...
de_mean = xr.open_dataset('DATA/ensemble_anomalies_arctic_mean.nc', decode_cf=True)
de_pctl_05 = xr.open_dataset('DATA/ensemble_anomalies_arctic_pctl_05.nc', decode_cf=True)
de_pctl_50 = xr.open_dataset('DATA/ensemble_anomalies_arctic_pctl_50.nc', decode_cf=True)
de_pctl_95 = xr.open_dataset('DATA/ensemble_anomalies_arctic_pctl_95.nc', decode_cf=True)
...
#for k in range(n_stations):
    ...
    if n_stations > 1:
        if (da_continent_stationcodes.isin([station_code]).any()) | (ds_continent_stationcodes.isin([station_code]).any()):
            if (da_continent_stationcodes.isin([station_code]).any()):
                print('k='+str(k)+': '+station_code.zfill(6)+': normal')            
            else:
                print('k='+str(k)+': '+station_code.zfill(6)+': short-segment')
        else:
            continue
    station_code = dt_continent_stationcodes[k].zfill(6)
    ...
    if (k>9) & (k%10 == 0):    
        ...
        if plot_gamma_fit == True:
            ...
            figstr = 'arctic_segment_anomalies_model_bias'+'_'+str(k).zfill(3)+'.png'
            titlestr = 'Arctic: 20CRv3 v GloSAT v short-segment model biases'    
        ...
        if plot_continent_mean == True:
            ...
            figstr = 'arctic_20crv3_model'+'_'+str(k).zfill(3)+'.png'
            titlestr = 'Arctic: 20CRv3 v GloSAT v short-segment model estimate: N(additional stations)='+str(n_model-n_GloSAT)
...
if plot_gamma_fit == True:
    ...
    if n_stations > 1:    
        figstr = 'arctic_segment_anomalies_model_bias.png'
        titlestr = 'Arctic: 20CRv3 v GloSAT v short-segment model biases'    
    else:
...
if plot_continent_mean == True:
    ...
    if n_stations > 1:        
        figstr = 'arctic_20crv3_model.png'
        titlestr = 'Arctic: 20CRv3 v GloSAT v short-segment model estimate: N(additional stations)='+str(n_model-n_GloSAT)
    else:
