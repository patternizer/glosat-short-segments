...
#station_code = '039172' # Armagh Observatory
station_code = '030882'  # Ben Nevis
...
if plot_gamma_fit == True:
    ...
    diff_20CRv3 = df['short_segment_20CRv3_anomaly'] - df['short_segment_station_anomaly']    
    diff_GloSAT = df['short_segment_anomaly'] - df['short_segment_station_anomaly'] 
    diff_model = df['short_segment_20CRv3_anomaly'] - df['short_segment_anomaly']
        
    diff_20CRv3 = np.array(diff_20CRv3).flatten()
    diff_GloSAT = np.array(diff_GloSAT).flatten()        
    diff_model = np.array(diff_model).flatten()        
    mask_20CRv3 = np.isfinite(diff_20CRv3) 
    mask_GloSAT = np.isfinite(diff_GloSAT) 
    mask_model = np.isfinite(diff_model) 
    diff_20CRv3 = diff_20CRv3[mask_20CRv3]
    diff_GloSAT = diff_GloSAT[mask_GloSAT]
    diff_model = diff_model[mask_model]
            
    bias1 = np.nanmean(diff_20CRv3)       
    bias2 = np.nanmean(diff_GloSAT)        
    bias3 = np.nanmean(diff_model)        
    
    if np.isfinite(bias1) == True:
        z_map1, gamma_pdf1, param1 = gamma_fit(diff_20CRv3)
    else:
        z_map1 = np.nan; gamma_pdf1 = np.nan; param1 = np.nan
    if np.isfinite(bias2) == True:        
        z_map2, gamma_pdf2, param2 = gamma_fit(diff_GloSAT)
    else:
        z_map2 = np.nan; gamma_pdf2 = np.nan; param2 = np.nan
    if np.isfinite(bias3) == True:                
        z_map3, gamma_pdf3, param3 = gamma_fit(diff_model)
    else:
        z_map3 = np.nan; gamma_pdf3 = np.nan; param3 = np.nan
    ...
    plt.axvline(x=bias1, lw=1, ls='dashed', color='red', label='bias='+str(np.round(bias1,2)).zfill(2)+'$\mathrm{\degree}C$'+': N(pairs)='+str(diff_20CRv3.shape[0]))
    plt.axvline(x=bias2, lw=1, ls='dashed', color='blue', label='bias='+str(np.round(bias2,2)).zfill(2)+'$\mathrm{\degree}C$'+': N(pairs)='+str(diff_GloSAT.shape[0]))
    plt.axvline(x=bias3, lw=1, ls='dashed', color='green', label='bias='+str(np.round(bias3,2)).zfill(2)+'$\mathrm{\degree}C$'+': N(pairs)='+str(diff_model.shape[0]))
    axes = plt.gca()
